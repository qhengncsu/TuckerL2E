...
times_cpopt = zeros(9,50);
times_tals = zeros(9,50);
times_brtf = zeros(9,50);
times_rpca = zeros(9,50);
times_horpcas = zeros(9,50);
times_horpcac = zeros(9,50);
times_rgrad = zeros(9,50);
times_l2e = zeros(9,50);
for i=1:9
  for j=1:50
    rng(i*50+j,'philox')
    info = create_problem('Type', 'CP', 'Size',[50 50 50],'Num_Factors',5*i,'Noise', 0.1,'M',0.0,'Factor_Generator','randn');
    X = info.Data;
    sz = size(X);
    ...
    times_rgrad(i,j) = cputime - tStart;
    ...
    times_horpcac(i,j) = cputime - tStart;
...
result = [mean(times_cpopt,2) mean(times_brtf,2) mean(times_horpcac,2) mean(times_rgrad,2) mean(times_l2e,2)]
csvwrite('time_cp_gross_dense.csv',result)