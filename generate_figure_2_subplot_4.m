...
rng(1,'philox')
reldiffs_tals = zeros(9,1);
reldiffs_horpcas = zeros(9,1);
...
for i=1:9
    ...
    reldiffs_tals(i) = norm(truth-tensor(Xhat_tals))/norm(truth);
    ...
    reldiffs_horpcas(i) = norm(truth-tensor(Xhat_horpcas))/norm(truth);
...
line1 = plot(ranks,reldiffs_tals,'-*','DisplayName','HOOI','color','#77AC30','LineWidth',1.5);
hold on
line2 = plot(ranks,reldiffs_horpcas,'-vr','DisplayName','HoRPCA-S','LineWidth',1.5);
hold on
